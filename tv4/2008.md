...
.notes everybody in the entire office may not know what those green boxes mean but everybody knows red means
something broken.
...
.notes CMS - takes about 30 minutes to do release and cannont publish during deploy. for a news site that can be a catastrophe.
wordpress one plugin between having not having a site.
Mule some systems just can't be deployed at will. Avoided db migrations so far but so what. even with db migrations it cane be done.
